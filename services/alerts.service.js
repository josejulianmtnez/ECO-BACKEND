	...
	actions: {
			...
			async handler(ctx) {
				try {
					...
					return alerts.map((alert) => ({
						id: alert.id,
						id_user_tutor: alert.id_user_tutor,
						id_user_child: alert.id_user_child,
						tutor_name: alert.tutor.name,
					...
					throw new Error(
						`Error al obtener todas las alertas: ${err.message}`
					);
				}
			},
		},
		get_by_id: {
			...
			async handler(ctx) {
				try {
					...
					throw new Error(
						`Error al obtener la alerta por ID: ${err.message}`
					);
				}
			},
		},
				...
				try {
					...
					if (!cols.includes(ctx.params.argument)) {
						throw new Error(
							`El argumento ${
								ctx.params.argument
							} no es válido. Argumentos válidos: ${cols.join(
								", "
							)}`
						);
					}
					...
					throw new Error(
						`Error al obtener la alerta por argumento: ${err.message}`
					);
				}
			},
		},
		new: {
			...
			async handler(ctx) {
				try {
					...
					throw new Error(
						`Error al crear nueva alerta: ${err.message}`
					);
				}
			},
		},
		update: {
			...
			async handler(ctx) {
				...
				ctx.params.id_user_child &&
					(data.id_user_child = ctx.params.id_user_child);
				...
				try {
					...
					throw new Error(
						`Error al actualizar la alerta: ${err.message}`
					);
				}
			},
		},
		delete: {
			...
			async handler(ctx) {
				if (!ctx.params.confirm) {
					return {
						message:
							"Por favor, confirme la eliminación estableciendo 'confirm' a true.",
					};
				}
				try {
					...
					throw new Error(
						`Error al eliminar la alerta: ${err.message}`
					);
				}
			},
		},
